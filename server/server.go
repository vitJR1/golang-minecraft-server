...
type ClientConnection struct {
	...
	// the mutex; but multiple goroutines (readLoop + keepAlive) write, and the
	// CFB8 stream cipher state would desync under concurrent writes.
	writeMu    sync.Mutex
	conn       net.Conn
	state      State
	playerName string
...
func (c *ClientConnection) handleReadError(err error) {
	...
	//	fmt.Printf("Read timeout for %s, but server alive\n", c.playerName)
	//	// Не закрываем соединение при таймауте
	//	return

	default:
		if opErr, ok := err.(*net.OpError); ok {
			if opErr.Err.Error() == "use of closed network server" {
				return
			}
		}
...
func (c *ClientConnection) handleStatus(packet *bytes.Buffer, packetID int) error {
	switch packetID {
	case SbStatusRequest: // Status Request
		...
		return c.safeWrite(CbStatusResponse, response)

	case SbStatusPing: // Ping
...
func (c *ClientConnection) handleLogin(packet *bytes.Buffer, packetID int) error {
	...
	case SbLoginStart: // Login Start
		...
		if banned := ban.IsBanned(c.playerName); banned != nil {
			msg := []byte(`{"text":"You are banned from this server.\nReason: ` + banned.Reason + `. Expires: ` + banned.ExpiresAt.Format(time.RFC1123) + `"}`)
			if err := c.safeWrite(CbLoginDisconnect, utils.WriteString(string(msg))); err != nil {
		...
		if cfg.OnlineMode {
			// TODO fix online mode, encryption problems
			fmt.Printf("Online mode enabled, will verify player %s with Mojang\n", c.playerName)
			verifyToken := make([]byte, 4)
			_, _ = rand.Read(verifyToken)

			// Encryption Request (login, 0x01)
			payload := make([]byte, 0, 64)

			// Server ID: vanilla sends "".
			payload = append(payload, utils.WriteString(cfg.ServerId)...)

			fmt.Println("lenght: ", len(utils.WriteString(cfg.ServerId)), len(publicKey), len(verifyToken))
			...
			// Should authenticate (Boolean) — ОБЯЗАТЕЛЬНО
			payload = append(payload, 0x01) // true (online-mode); 0x00 если false
			...
			if err != nil {
				...
				err := c.safeWrite(0x00, utils.WriteString(string(msg)))
				if err != nil {
					return fmt.Errorf("sending auth failure message: %w", err)
				}
			...
			encryptStream := encryption.NewCFB8Encrypt(block, sharedSecret)
			decryptStream := encryption.NewCFB8Decrypt(block, sharedSecret)
			c.writeMu.Lock()
			c.conn = encryption.WrapEncryptedConn(c.conn, encryptStream, decryptStream)
			c.writeMu.Unlock()
			fmt.Println("Encrypted server established")

			// Отправляем Login Success
			parsedUUID, err := utils.FormatUUID(profile.ID)
			if err != nil {
			...
			uuidBytes, err := utils.WriteUUID(parsedUUID)
			if err != nil {
				return fmt.Errorf("writing UUID: %w", err)
			}
			payload = make([]byte, 0, 64)
			...
			payload := []byte{}
			uuidBytes, err := utils.WriteUUID(uuid)
			if err != nil {
				return fmt.Errorf("writing UUID: %w", err)
			}
			payload = append(payload, uuidBytes...)
...
func (c *ClientConnection) handlePlay(packet *bytes.Buffer, packetID int) error {
	...
	switch packetID {
	case SbPlayKeepAlive: // Keep Alive response
	...
	case SbPlayChatMessage: // Chat message
		...
		if !c.isClosed() {
			...
			c.safeWrite(CbPlaySystemChat, utils.WriteString(response))
		}

	case SbPlaySetPos: // X Y Z OnGround
	...
	case SbPlaySetPosRot: // X Y Z Yaw Pitch OnGround
	...
	case SbPlaySetRot: // Yaw Pitch OnGround
		yaw, pitch, onGround, err := readYawPitchOnGround(packet)
		if err != nil {
	...
	case SbPlayTeleportConfirm: // Teleport Confirm (serverbound)
		id, _ := utils.ReadVarInt(packet)
		fmt.Println("Teleport confirmed:", id)

	case SbPlayClientInfo:
		// Sent by the client right after LoginSuccess. We don't act on any of
		// the fields yet (locale, view distance, chat mode, displayed skin
		// parts, main hand, etc.) — just consume to keep the buffer clean.
...
//	payload = append(payload, utils.WriteVarInt32(3)...)
//
